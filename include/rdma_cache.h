...
// 缓存分配比例常量
constexpr float QP_CACHE_RATIO = 0.5f; // QP缓存比例50%
constexpr float CQ_CACHE_RATIO = 0.2f; // CQ缓存比例20%
constexpr float MR_CACHE_RATIO = 0.3f; // MR缓存比例30%
constexpr float PD_CACHE_RATIO = 0.1f; // PD缓存比例10%
...
public:
  explicit RdmaCache(size_t total_cache_size) {
    // 使用预定义的常量比例分配缓存大小
    size_t qp_cache_size =
        static_cast<size_t>(total_cache_size * QP_CACHE_RATIO);
    size_t cq_cache_size =
        static_cast<size_t>(total_cache_size * CQ_CACHE_RATIO);
    size_t mr_cache_size =
        static_cast<size_t>(total_cache_size * MR_CACHE_RATIO);
    size_t pd_cache_size =
        static_cast<size_t>(total_cache_size * PD_CACHE_RATIO);
  ...
  void set_qp_info(uint32_t qp_num, const QPValue &info);
